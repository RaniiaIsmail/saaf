...
export class TransactionsComponent {
  ...
  loadAll() {
    ...
    // جيب بيانات الـ master wallet info (الرصيد) بشكل منفصل
    this.transactionService.getMasterWalletTransactions().subscribe({
      next: (res) => {
        this.masterWalletInfo = res.masterWallet
        const balanceSAR =
          res.masterWallet?.balances?.find((b: any) => b.currency === 'SAR')?.amount ?? 0
  ...
  updateStats() {
    ...
    const balanceSAR =
      this.masterWalletInfo?.balances?.find((b: any) => b.currency === 'SAR')?.amount ?? 0
    this.stateList[2].amount = `${balanceSAR.toLocaleString()} SAR`
  }
  ...
  get masterWalletBalance(): string {
    ...
    const sar = this.masterWalletInfo.balances?.find((b: any) => b.currency === 'SAR')?.amount ?? 0
